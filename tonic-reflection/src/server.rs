...
#[derive(Debug, Error)]
pub enum Error {
    /// An error was encountered decoding a `prost_types::FileDescriptorProto` from a buffer.
    #[error("error decoding FileDescriptorProto from buffer")]
    DecodeError(#[from] prost::DecodeError),
    /// A `prost_types::FileDescriptorProto`
    #[error("invalid FileDescriptorProto - {0}")]
    InvalidFileDescriptorProto(String),
}
...
pub struct Builder {
    ...
    encoded_file_descriptors: Vec<&'static [u8]>,
...
impl Builder {
    ...
    pub fn configure() -> Self {
        Builder {
            ...
            encoded_file_descriptors: Vec::new(),
    ...
    /// Registers an instance of `prost_types::FileDescriptorProto` with the gRPC Reflection
    /// Service builder.
    pub fn register_file_descriptor(mut self, file_descriptor: FileDescriptorProto) -> Self {
        self.file_descriptors.push(file_descriptor);
        self
    }

    /// Registers a byte slice containing an encoded `prost_types::FileDescriptorProto` with
    /// the gRPC Reflection Service builder.
    pub fn register_encoded_file_descriptor(
        mut self,
        encoded_file_descriptor: &'static [u8],
    ) -> Self {
        self.encoded_file_descriptors.push(encoded_file_descriptor);
        self
    }
    ...
    pub fn build(mut self) -> Result<ServerReflectionServer<impl ServerReflection>, Error> {
        if self.include_reflection_service {
            self = self.register_encoded_file_descriptor(crate::proto::FILE_DESCRIPTOR);
        }

        for encoded in &self.encoded_file_descriptors {
            let decoded = FileDescriptorProto::decode(*encoded)?;
            self.file_descriptors.push(decoded);
        }

        let fds = self.file_descriptors.clone();
        let mut processed: HashSet<String> = HashSet::new();
        for fd in fds {
            let name = match fd.name.clone() {
                None => {
                    return Err(Error::InvalidFileDescriptorProto(
    ...
    fn process_file(&mut self, fd: FileDescriptorProto) -> Result<(), Error> {
        ...
        // TODO(jen20): work out how to handle dependencies
...
fn extract_name(
    ...
    match maybe_name {
        None => Err(Error::InvalidFileDescriptorProto(format!(
            "missing {} name",
            name_type
        ))),
        Some(name) => {
